...
if confirm(message="Want to use the graphical interface?"):
    matplotlib.use('WX')
else:
    matplotlib.use('Agg')
...
if confirm(message="Use an existing model?") == True:
    source = genfromtxt(openFile(message="Select CSV", wildcard="*.csv"), delimiter=',', skip_header=0)
    source = dh.preProcessCytofData(source)

    preprocessor = prep.StandardScaler().fit(source)
    source = preprocessor.transform(source)

    trainingPath = openFile('Select a training file', wildcard="*.json")

    calibMMDNet = model_from_json(open(trainingPath, "r").read())
    calibMMDNet.load_weights(openFile('Select a training weights', wildcard="*.h5"))
...
targetPath = openFile(message="Select target CSV (baseline)", wildcard="*.csv")
sourcePath = openFile(message="Select source CSV (data to calibrate)", wildcard="*.csv")
...
calibMMDNet.fit(source, sourceLabels, epochs=10, batch_size=1000, validation_split=0.1, verbose=1,
...
# save models
file = open(saveFile('Save the model', wildcard="*.json"), "w+")
file.write(calibMMDNet.to_json())
file.close()
